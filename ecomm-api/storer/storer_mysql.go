...
func createOrder(ctx context.Context, tx *sqlx.Tx, o *Order) (*Order, error) {
	...
	id, err := res.LastInsertId()
	if err != nil {
		return nil, fmt.Errorf("error getting last id %w", err)
	}
	o.ID = id
	return o, nil
}
...
func (ms *MySQLStorer) GetOrder(ctx context.Context, id int64) (*Order, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error getting orderitem %w", err)
	}
	o.Items = items
	return &o, nil
}

func (ms *MySQLStorer) ListOrder(ctx context.Context) ([]Order, error) {
	var orders []Order
...
func (ms *MySQLStorer) DeleteOrder(ctx context.Context, id int64) error {
	err := ms.execTx(ctx, func(tx *sqlx.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("error deleting orderitem %w", err)
		}
...
func (ms *MySQLStorer) execTx(ctx context.Context, fn func(*sqlx.Tx) error) error {
	...
	if err != nil {
		if rbErr := tx.Rollback(); rbErr != nil {
			return fmt.Errorf("error rolling back tansaction %w", rbErr)
		}
